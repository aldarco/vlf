...
class dsp:
    def __init__(self, config):
        ...
        self.clipping_filter = config["clipping"]
    ...
    def get_amplitudes(self, x):
        ...
        if self.clipping_filter: [x[:,0] , x[:,1]] = self.IQ_clipping_filter(x)
    ...
    def fft_method(self, st, wf=np.blackman):
        #return fft_pavnet.fft_window(st, wlen=fft_npts,fw=signal.windows.flattop)
        return fft_pavnet.fft_overlap(st, fft_npts=self.fft_npts,window=wf)
